...
export const displayMap = (mapObj, bounds, data) => {
  data.forEach(d => {
  ...
  const update = zoom => {
    ...
    let zoomLevel = zoom || parseInt(mapObj.getZoom());
    ...
    clusters.forEach(cluster => {
      ...
      if (cluster.properties && cluster.properties.cluster) {
        ...
        el.classList.add(
          getClusterElementClass(cluster.properties.point_count)
        );
    ...
    markers.forEach(marker => marker.addTo(mapObj));
  };
